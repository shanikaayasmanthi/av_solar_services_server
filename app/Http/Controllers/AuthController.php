...
//         $frontendResetUrl = config('app.frontend_url') ?? env('FRONTEND_URL', null);
//         if (!$frontendResetUrl) {
...
//         $webResetUrl = rtrim($frontendResetUrl, '/') . '/reset-password?token=' . urlencode($plainToken) . '&email=' . urlencode($email);
//         // mobile deep link (Flutter)
//         $mobileResetUrl = "myapp://reset-password?token=" . urlencode($plainToken) . "&email=" . urlencode($email);
...
//     return response()->json([
//         'status' => 'error',
//         'message' => 'Could not send reset email. Check logs for details.'
//     ], 500);
// }
